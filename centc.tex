...
We found a total of 12,162 CentC variants in the maize reference genome and unassembled BACs.  Of these, 8,259 were unique over their full length. No CentC sequence occurred more than 10 times in the genome, and the vast majority ($>$75\%, Table \ref{supp.occurence}) of non-unique CentC variants occurred only twice.  Of the 2,266 non-unique CentC sequences, only 3 were tandem, identical duplicates.  Excluding the 1,523 copies that occur on the unassembled BACs, genome-wide CentC locations also show that nearly all of the 10,639 CentC copies are found in one of the 248 clusters identified on chromosomes 1-10; only 14 occurred as solo copies.  Clusters varied in width from single CentC copies to 84KB with a mean of $\sim$7KB ($\sim$45 CentC copies). Chromosomes varied greatly in CentC copy number, though we know that centromere assemblies for all of the chromosomes are not complete.   For example, CENH3-ChIP sequence from an oat-maize addition line including only one maize chromosome \citep{kynast2001} nonetheless has many reads that map to the unassembled BACs (Table \ref{supp.gaby}).  In particular, chromosome 6 had many more reads aligning to the unassembled BACs than it did to its own centromere, suggesting a particularly incomplete assembly.  Examining total repeat number, chromosome 7 had the most CentC, with 3,200 copies, while chromosome 6 had the fewest with 32 copies.
...
In addition to CentC, previous studies have described Cent4, a second high copy number centromere repeat specific to maize chromosome 4 \citep{Page2001}. 
Available evidence does not point to an important centromere function for Cent4: CenH3 chromatin immunoprecipitation data \citep{Wolfgruber2009} show no significant over-representation of Cent4 compared to five known non-centromeric TE’s, and Cent4 probes lag behind CentC probes in cell division \citep{Jiang2002}.  BLAST analyses of Cent4 sequences from Genbank revealed high homology to the poorly characterized LTR retrotransposon RLX\_sela, which was previously shown to be associated with heterochromatic  knobs \citep{Tenaillon2011, Chia2012}.  
...
\jri{check tense throughout. i think some is past and some is present} 
...
Nonetheless, while we can cluster the repeats into groups of related sequences, these groups seem to have little relation to current or ancient maize chromosomes (Figure \ref{pcoa}).  
We see no evidence of chromosome specific repeats such as that seen in X and Y \citep{Kawabe2005, Pontes2004}, suggesting the presence of mechanisms homogenizing repeats across centromeres on different chromosomes. We further show that Cent4, while specific to chromosome 4, appears to be a poorly characterized tandem repeat or retroelement, but is not associated with the centromere.  

Instead of chromosome-specific groups, we find that virtually all the large arrays of CentC in the maize  genome derive from the maize1 subgenome, which is known to have lower gene loss and higher average expression than the maize subgenome 2 \cite{Schnable2011}. \jri{is it maize1 or maize subgenome 1? you use both here} 
One possible explanation for this clear bias is that differential expression between subgenomes affects centromere function.  
Regulatory microRNA’s corresponding to centromeric repeats have been observed\citep{ReinhartBartel2002}, and perhaps the silenced centromere became more susceptible to deletion just like other parts of subgenome2.  
We see no evidence of a more rapid decay of CentC sequences in subgenome2, but we likely have poor ability to detect deletion of entire tandem arrays.  
An alternative explanation is may simply be that the subgenome1 parent had more or larger CentC arrays, leading to an unequal representation in the ancient polyploid ancestor of maize.
...
%Our study traces the changes in centromere repeat genetic relatedness across maize chromosomes to show how ancient tetraploidy and subsequent evolution has impacted centromeres.  Most interestingly, we show that 

%!RELATEDNESS
Our sequence comparison of CentCs also enabled us to explore the relationship between genetic and physical distance between repeats.  
Using the well-assembled centromeres on chromosomes 2 and 5, we found spatial autocorrelation among repeats, and we observe genome-wide that many CentC’s within an array fall into the same genetic cluster.  
These observations are consistent with the simple idea that most repeats arise due to tandem duplication, and long distance transposition of CentC, while necessary to homogenize repeats across chromosomes \citep{Shi2010}, is relatively uncommon.

One unusual result from our sequence comparison was the finding that pairs of CentC on different chromosomes may be identical or nearly so.  Our simulations suggest that this is plausible due to homoplasmy alone under realistic assumptions about mutation rate and divergence time.  We speculate that the vast majority of the CentC’s exist as a result of old duplications and that mutations have had a long time to accumulate. Roughly 80\% of the CentC repeats have their closest genetic relative on the same chromosome, as expected under a model of tandem duplication.  However, only 14\% of closest genetic pairs are found within 10KB of each other, suggesting that their tandem duplication is old and that most CentC’s have persisted within the genome for a long time. 
