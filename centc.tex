...
\jri{check tense throughout. i think some is past and some is present. also fill in species names X and Y and add CITEs} 
...
We see no evidence of chromosome specific repeats such as that seen in X and Y \citep{Kawabe2005, Pontes2004}, suggesting the presence of a mechanisms that homogenizes repeats across centromeres on different chromosomes. 
We further show that Cent4, once thought to be a chromosome-specific centromere repeat (CITE), appears to be a poorly characterized tandem repeat or nonautonomous retroelement, but is not associated with the centromere.  
...
One possible explanation for this result is that centromere inactivation mirrors the differential expression seen between the subgenomes.  
...
Large CentC clusters are clearly biased in favor of subgenome 1, which is known to have lower gene loss and higher average expression than maize subgenome 2 \cite{Schnable2011}. 
Consistent with this idea, \citet{ReinhartBartel2002} find evidence of regulatory microRNA’s corresponding to centromeric repeats.   
Of course, an alternative explanation is simply that the subgenome 1 parent had more or larger CentC arrays, leading to an initial unequal representation in the ancient polyploid ancestor of maize.
